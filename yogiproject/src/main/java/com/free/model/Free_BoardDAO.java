...
public class Free_BoardDAO {
	...
	public List<Free_BoardVO> getFrees(int start, int end) {
		...
		try {
			...
			String sql = "select * from (select rownum rnum, no, title, writer, pass, content, readcount, ref, step, depth, regdate "
					+ "from (select * from proprac_free order by ref desc, step asc)) " + "where rnum>=? and rnum<=?";
			pstmt = conn.prepareStatement(sql);
	 ...
	 * 글 삭제
	 * 
	 * 글 삭제 처리를 하려면 입력받은 비밀번호와 데이터베이스에 저장된 비밀번호를 비교하여 같으면 삭제처리한다. 글 삭제 성공 : 1 비밀번호 :
	 * 0 삭제 글이 존재하지 않을 경우 : -1
	 */
	...
	public int getFreeCount(String find, String find_box) {
		...
		try {
			...
			if (find.equals("writer")) {
				...
				pstmt = conn
						.prepareStatement("select count(*) from proprac_free where title like '%" + find_box + "%'");
			} else if (find.equals("content")) {
				pstmt = conn
						.prepareStatement("select count(*) from proprac_free where content like '%" + find_box + "%'");
			} else {
	...
	public List<Free_BoardVO> getFrees(String find, String find_box, int start, int end) {
		...
		try {
			...
			sql.append(
					"(select rownum rnum, no, title, writer, pass, content, readcount, ref, step, depth, regdate from ");
			if (find.equals("writer")) {
				sql.append("(select * from proprac_free where writer like '%" + find_box
						+ "%' order by ref desc, step asc, no desc)) where rnum>=? and rnum<=?");
				pstmt = conn.prepareStatement(sql.toString());
				pstmt.setInt(1, start);
				pstmt.setInt(2, end);
			} else if (find.equals("title")) {
				sql.append("(select * from proprac_free where title like '%" + find_box
						+ "%' order by ref desc, step asc, no desc)) where rnum>=? and rnum<=?");
				pstmt = conn.prepareStatement(sql.toString());
				pstmt.setInt(1, start);
				pstmt.setInt(2, end);
			} else if (find.equals("content")) {
				sql.append("(select * from proprac_free where content like '%" + find_box
						+ "%' order by ref desc, step asc, no desc)) where rnum>=? and rnum<=?");
				pstmt = conn.prepareStatement(sql.toString());
				pstmt.setInt(1, start);
				pstmt.setInt(2, end);
				...
				sql.append(
						"(select * from proprac_free order by ref desc, step asc, no desc)) where rnum>=? and rnum<=?");
				pstmt = conn.prepareStatement(sql.toString());
				pstmt.setInt(1, start);
				pstmt.setInt(2, end);
			}
	...
	public ArrayList<Free_BoardVO> replyList(int no) {
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = "select * from proprac_free_reply where parentno=? order by ctime desc";
	...
	public int deleteReply(int cno) {
		...
		PreparedStatement pstmt = null;
		int result=0;
		String sql = "delete from proprac_free_reply where cno="+cno;
